...
values ('1800000000001000003',
        ...
        jsonb_build_object(
                ...
                'description',
                '默认示例：经 MCP 调用本机内置 format_line（模板 {who}/{what}）。请按实际服务地址修改 endpoint。',
                'parameters', jsonb_build_object(
                        'type', 'object',
                        'properties', jsonb_build_object(
                                'template',
                                jsonb_build_object('type', 'string', 'description', '含 {who}、{what} 的模板'),
