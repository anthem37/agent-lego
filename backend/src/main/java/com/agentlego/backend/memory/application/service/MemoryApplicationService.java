package com.agentlego.backend.memory.application.service;
...
public class MemoryApplicationService {
    ...
    private final MemoryDtoMapper memoryDtoMapper;

    public MemoryApplicationService(MemoryRepository repository, MemoryDtoMapper memoryDtoMapper) {
        this.repository = repository;
        this.memoryDtoMapper = memoryDtoMapper;
    }
    ...
    public MemoryQueryResponse query(MemoryQueryRequest req) {
        ...
        resp.setItems(items.stream().map(memoryDtoMapper::toDto).toList());
        return resp;
    }
}
